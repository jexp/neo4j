...
class ProfilerAcceptanceTest extends ExecutionEngineFunSuite with CreateTempFileTestSupport {
  ...
  test("tracks number of rows") {
    //GIVEN
    createNode("foo" -> "bar")
    val result = myProfile("START n = node(0) RETURN n")
  ...
  test("tracks number of graph accesses") {
    //GIVEN
    createNode("foo" -> "bar")
    val result = myProfile("START n = node(0) RETURN n.foo")
  ...
  test("tracks graph global queries") {
    ...
    //GIVEN
    val result = myProfile("START n=node(*) RETURN n.foo")
  ...
  test("tracks optional matches") {
    //GIVEN
    createNode()
    val result = myProfile("start n=node(*) optional match (n)-->(x) return x")
  ...
  test("should produce profile when using limit") {
    ...
    createNode()
    createNode()
    val result = myProfile("""START n=node(*) RETURN n LIMIT 1""")
