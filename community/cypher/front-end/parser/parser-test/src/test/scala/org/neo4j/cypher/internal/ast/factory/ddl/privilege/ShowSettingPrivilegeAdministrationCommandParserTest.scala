...
class ShowSettingPrivilegeAdministrationCommandParserTest extends AdministrationAndSchemaCommandParserTestBase {
          ...
          test(s"$verb$immutableString SHOW SOME SETTING * ON DATABASE * $preposition role") {
            ...
            failsParsing[Statements].in {
              ...
              case _ => _.withSyntaxErrorContaining(
                  s"""Invalid input 'SOME': expected 'ALIAS', 'CONSTRAINT', 'CONSTRAINTS', 'INDEX', 'INDEXES', 'PRIVILEGE', 'ROLE', 'AUTH RULE', 'SECRETS', 'SERVER', 'SERVERS', 'SETTING', 'SETTINGS', 'TRANSACTION', 'TRANSACTIONS' or 'USER' (line 1, column ${offset + 1} (offset: $offset))""".stripMargin
                )
            }
          }
      }
  ...
  private def settingQualifier(glob: String, position: InputPosition = pos): SettingQualifier =
    SettingQualifier(glob)(position)
}
