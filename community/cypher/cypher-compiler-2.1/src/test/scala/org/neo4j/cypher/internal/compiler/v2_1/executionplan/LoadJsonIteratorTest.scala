...
class LoadJsonIteratorTest extends CypherFunSuite {

  val url = new URL("file://uselessinfo.json")
  val inner = Seq(Map("key" -> "value"), Map("key" -> "anotherValue"))

  test("should provide information about the current row") {
    val it = new LoadJsonIterator(url, inner.iterator)(())
  ...
  test("should call onNext when next is called") {
    var called = false
    val it = new LoadJsonIterator(url, inner.iterator)({ called = true })
  ...
  test("should call the inner iterator when calling hasNext") {
    var called = false
    val inner = new Iterator[Map[String, Object]] {
    ...
    val it = new LoadJsonIterator(url, inner)(())
    called should equal(false)
    it.hasNext
    called should equal(true)
  }
