...
CodeMirror.defineMode "cypher", (config) ->
  ...
  keywords = wordRegexp(["start", "merge", "load", "csv", "json", "using", "periodic commit", "on create", "on match", "match", "index on", "drop", "where", "with", "limit", "skip", "order", "by", "return", "create", "delete", "set", "unique", "unwind"])
  operatorChars = /[*+\-<>=&|~]/
  startState: (base) ->
    tokenize: tokenBase
    context: null
